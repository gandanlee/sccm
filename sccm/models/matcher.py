...
from sccm.utils import get_tuple_transform_ops
from sccm.utils.local_correlation import local_correlation
from sccm.utils.utils import (
    check_rgb,
    cls_to_flow_refine,
    get_autocast_params,
    check_not_i16,
)
from sccm.utils.kde import kde
from sccm.models.encoders import CNNandDinov2


def wrap_normalized_longitude_grid(grid: torch.Tensor) -> torch.Tensor:
    u = grid[..., 0]
    u_wrapped = torch.where(
        (u < -1.0) | (u > 1.0),
        torch.remainder(u + 1.0, 2.0) - 1.0,
        u,
    )
...
class ConvRefiner(nn.Module):
    def __init__(
        ...
        self.cyclic_local_corr = bool(cyclic_local_corr)
        self.cyclic_x_hat = bool(cyclic_x_hat)
    ...
    def forward(self, x, y, warp, scale_factor=1, logits=None):
        ...
        with torch.autocast(
            ...
            x_hat_grid = warp.permute(0, 2, 3, 1)
            if self.cyclic_x_hat:
                x_hat_grid = wrap_normalized_longitude_grid(x_hat_grid)
            x_hat = F.grid_sample(
                y, x_hat_grid, align_corners=False, mode=self.sample_mode
            )
                ...
                if self.local_corr_radius:
                    if self.corr_in_other:
                        ...
                        local_corr = local_correlation(
                            ...
                            cyclic_horizontal=self.cyclic_local_corr,
                        )
                    else:
...
class Decoder(nn.Module):
    def __init__(
        ...
        amp_dtype=torch.float16,
        gacfine_enabled=False,
        gacfine_gamma_init=0.0,
        gacfine_gamma_max=4.0,
        gacfine_warmup_steps=30000,
        gacfine_scales=(8, 4, 2, 1),
    ):
        super().__init__()
        ...
        self.amp_dtype = amp_dtype
        self.store_refiner_aux_for_eval = False

        # ── GAC-fine: per-scale learnable γ + auto-incremented step buffer ──
        self.gacfine_enabled = bool(gacfine_enabled)
        self.gacfine_gamma_max = float(gacfine_gamma_max)
        self.gacfine_warmup_steps = int(gacfine_warmup_steps)
        self.gacfine_scales = tuple(int(s) for s in gacfine_scales)
        if self.gacfine_enabled:
            import math as _math
            target = float(gacfine_gamma_init)
            if target < 1e-4:
                raw_init = -10.0
            else:
                raw_init = _math.log(_math.expm1(target))
            self.gacfine_gamma_raws = nn.ParameterDict({
                str(s): nn.Parameter(torch.tensor(raw_init, dtype=torch.float32))
                for s in self.gacfine_scales
            })
            self.register_buffer("_gacfine_step",
                                  torch.tensor(0, dtype=torch.long))
        else:
            self.gacfine_gamma_raws = None
        # Diagnostics — populated during forward, read by training loop
        self._last_gacfine = {}
    ...
    def forward(
        ...
        for new_scale in all_scales:
            ...
            if ins in coarse_scales:
                ...
                candidate_hyp = getattr(self.gps[new_scale], "_last_topk_coords", None)
                if candidate_hyp is not None:
                    smh_hypotheses = candidate_hyp
                ...
                if getattr(self, "gcas_enabled", False):
                    flow = self.gcas(f1_s, flow)

                # ESFD #6: learnable sparse anchor-centered zoom refinement (on detached coarse flow)
                _zr = getattr(self, "zoom_refine", None)
                if _zr is not None and getattr(self, "_zoom_imgs", None) is not None and ins in coarse_scales:
                    flow = _zr(flow, self._zoom_imgs[0], self._zoom_imgs[1])

                # ── S²-SCL: sphere-tangent sub-cell localization of the coarse anchor ──
                # (identity when zero-init / absent; attached post-construction in get_model)
                if getattr(self, "subcell", None) is not None and ins in getattr(self, "subcell_scales", ()):
                    flow = self.subcell(f1_s, f2_s, flow)

            if new_scale in self.conv_refiner:
                corresps[ins].update(
                    {"flow_pre_delta": flow}
                ) if (self.training or self.store_refiner_aux_for_eval) else None
                refiner = self.conv_refiner[new_scale]
                if smh_hypotheses is not None and hasattr(refiner, "set_smh_hypotheses"):
                    B_h, K_h, _, H_h, W_h = smh_hypotheses.shape
                    hyp = smh_hypotheses.reshape(B_h * K_h, 2, H_h, W_h).to(flow.device)
                    hyp = F.interpolate(
                        hyp,
                        size=f1_s.shape[-2:],
                        mode="bilinear",
                    ...
                    hyp = hyp.reshape(B_h, K_h, 2, f1_s.shape[-2], f1_s.shape[-1])
                    refiner.set_smh_hypotheses(hyp)
                delta_flow, delta_certainty = refiner(
                    f1_s,
                    f2_s,
                    flow,
                ...
                # ── GAC-fine: gate delta_certainty using upsampled coarse d_geo ──
                if self.gacfine_enabled and ins in self.gacfine_scales:
                    # Find coarse matcher and read stored d_geo grid
                    d_geo_grid = None
                    for sc in self.gps:
                        gp = self.gps[sc]
                        candidate = getattr(gp, "_gacfine_d_geo_grid", None)
                        if candidate is not None:
                            d_geo_grid = candidate
                            break
                    if d_geo_grid is not None:
                        # Upsample (B, H_c, W_c) → (B, 1, H_fine, W_fine)
                        d_geo_up = F.interpolate(
                            d_geo_grid.unsqueeze(1).to(delta_certainty.dtype),
                            size=delta_certainty.shape[-2:],
                            mode='bilinear', align_corners=False,
                        )
                        # γ_fine with linear warmup + tanh upper-bound
                        if self.training:
                            self._gacfine_step += 1
                        gamma_raw = self.gacfine_gamma_raws[str(ins)]
                        gamma_unbounded = F.softplus(gamma_raw)
                        gamma_bounded = self.gacfine_gamma_max * torch.tanh(
                            gamma_unbounded / self.gacfine_gamma_max
                        )
                        cur_step = float(self._gacfine_step.item())
                        warmup_factor = min(1.0, cur_step / max(self.gacfine_warmup_steps, 1))
                        gamma_eff = warmup_factor * gamma_bounded
                        delta_certainty = delta_certainty - gamma_eff * d_geo_up
                        # Diagnostics per scale
                        self._last_gacfine[ins] = {
                            'gamma_eff': float(gamma_eff.detach().item()) if isinstance(gamma_eff, torch.Tensor) else float(gamma_eff),
                            'gamma_raw': float(gamma_raw.detach().item()),
                            'd_geo_mean': float(d_geo_up.mean().item()),
                            'd_geo_max':  float(d_geo_up.max().item()),
                            'warmup_factor': float(warmup_factor),
                        }
                refiner_aux = getattr(self.conv_refiner[new_scale], "_last_gsa_aux", None)
                if (self.training or self.store_refiner_aux_for_eval) and refiner_aux is not None:
                    delta_flow_reg_aux = refiner_aux["delta_flow_reg"]
                    delta_flow_gsa_aux = refiner_aux["delta_flow_gsa"]
                    gate_aux = refiner_aux["gsa_gate"]
                    reg_displacement_aux = ins * torch.stack(
                        (
                            delta_flow_reg_aux[:, 0].float() / (self.refine_init * w),
                            delta_flow_reg_aux[:, 1].float() / (self.refine_init * h),
                        ),
                        dim=1,
                    )
                    gsa_displacement_aux = ins * torch.stack(
                        (
                            (gate_aux[:, 0] * delta_flow_gsa_aux[:, 0]).float() / (self.refine_init * w),
                            (gate_aux[:, 0] * delta_flow_gsa_aux[:, 1]).float() / (self.refine_init * h),
                        ),
                        dim=1,
                    )
                    corresps[ins].update(
                        {
                            "gsa_flow_reg": flow + reg_displacement_aux,
                            "gsa_delta_norm": gsa_displacement_aux,
                            "gsa_gate": gate_aux,
                        }
                    )
                grv_aux = getattr(self.conv_refiner[new_scale], "_last_grv_aux", None)
                if (self.training or self.store_refiner_aux_for_eval) and grv_aux is not None:
                    delta_flow_reg_aux = grv_aux["delta_flow_reg"]
                    delta_flow_grv_aux = grv_aux["delta_flow_grv"]
                    reg_displacement_aux = ins * torch.stack(
                        (
                            delta_flow_reg_aux[:, 0].float() / (self.refine_init * w),
                            delta_flow_reg_aux[:, 1].float() / (self.refine_init * h),
                    ...
                    grv_displacement_aux = ins * torch.stack(
                        (
                            delta_flow_grv_aux[:, 0].float() / (self.refine_init * w),
                            delta_flow_grv_aux[:, 1].float() / (self.refine_init * h),
                        ),
                        dim=1,
                    )
                    corresps[ins].update(
                        {
                            "grv_flow_reg": flow + reg_displacement_aux,
                            "grv_delta_norm": grv_displacement_aux,
                            "grv_tangent_2d": grv_aux["grv_tangent_2d"],
                            **({"grv_gate": grv_aux["grv_gate"]} if "grv_gate" in grv_aux else {}),
                            **({"grv_gate_logits": grv_aux["grv_gate_logits"]} if "grv_gate_logits" in grv_aux else {}),
                        }
                    )
                strf_aux = getattr(self.conv_refiner[new_scale], "_last_strf_aux", None)
                if (self.training or self.store_refiner_aux_for_eval) and strf_aux is not None:
                    corresps[ins].update(
                        {
                            "strf_flow_anchor": flow,
                            "strf_chart_delta_norm": strf_aux["strf_chart_delta_norm"],
                            "strf_tangent_delta_norm": strf_aux["strf_tangent_delta_norm"],
                            "strf_tangent_2d": strf_aux["strf_tangent_2d"],
                            "strf_gate": strf_aux["strf_gate"],
                        }
                    )
                corresps[ins].update(
                    {
...
def _check_input(im_input):
    if isinstance(im_input, (str, os.PathLike)):
        ...
        assert H % 16 == 0, "im_input must be a multiple of 16"
        assert W % 16 == 0, "im_input must be a multiple of 16"
        im = im_input
    return im
...
class RegressionMatcher(nn.Module):
    ...
    def forward(self, batch, batched=True, upsample=False, scale_factor=1):
        # Expose the current batch to downstream modules that need GT (e.g.
        # FlowMatchingS2Matcher computing its Conditional Flow Matching loss).
        # We use both a thread-local *and* direct attribute injection on the
        # coarse matcher; the latter is a belt-and-braces backup for DDP.
        try:
            from sccm.models.flow_matching_s2_matcher import (
                set_cfm_batch, FlowMatchingS2Matcher,
            )
            set_cfm_batch(batch)
            try:
                gp16 = self.decoder.gps["16"]
                if isinstance(gp16, FlowMatchingS2Matcher):
                    gp16.set_batch(batch)
        ...
        if getattr(self.decoder, "zoom_refine", None) is not None and ("im_A" in batch):
            dv = f_q_pyramid[next(iter(f_q_pyramid))].device
            self.decoder._zoom_imgs = (batch["im_A"].to(dv), batch["im_B"].to(dv))
